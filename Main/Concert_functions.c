...
Musician** getMusiciansArrToConcert(Concert* concert, Musician*** musicians_collection_arr, int* concert_musicians_count)
{
	...
	while (concert_inst_node != NULL)
	{
		...
		{
			/* concert_musician is a pointers to musicians dynamic array. the musicians are pulled from musicians collection array into the concert
			musicians array if they are available and can play the requested instrument from the current node of the concert instrument list*/
